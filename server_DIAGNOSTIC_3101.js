...
//   http://YOUR_COMPUTER_IP:3000/tablet.html
//
// If the tablet cannot connect, check that both devices are on the same Wi-Fi,
// that the IP address is correct, and that the computer firewall allows port 3000.
//
...
const { recognizeFullWord, recogniseFullHandwrittenWord, recognitionModeLabel, getRecognitionStatus, emptyRecognition } = require("./recognition");
...
app.post("/api/test-gemini-recognition", async (req, res) => {
  try {
    ...
    const status = getRecognitionStatus();
    if (process.env.ENABLE_GEMINI_RECOGNITION !== "true") {
